...
module.exports = {
    async handler(chatUpdate) {
        ...
        global.locale = 'id'
        global.week = d.toLocaleDateString(locale, {
            weekday: 'long'
        })
    ...
    async participantsUpdate({
        ...
        switch (action) {
            ...
            case 'demote':
                ...
                if (chat.detect) await this.sendMessage(id, {
                    text: text,
                    mentions: [participants[0]],
                    jpegThumbnail: await (await fetch(blur)).buffer()
                }, {
                    quoted: {
                        key: {
                            fromMe: false,
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.sB(m.chat, msg.trim(), wm, null, [
        [`Menu`, `#menu`]
    ], m)
}
