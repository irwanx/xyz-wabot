...
export async function handler(chatUpdate) {
    ...
    global.set.image = await Func.sharp(global.set.img, 400)
    if (!m)
        return
    if (db.data == null)
        await loadDatabase()
    try {
        ...
        try {
            ...
            if (settings) {
                ...
                if (!('react' in settings)) settings.react = false
         ...
         */
        if (db.data.settings[this.user.jid].readsw) await this.readMessages([m.key])
