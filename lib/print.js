...
const { buffer } = (await import('terminal-image')).default 
const urlRegex = (await import('url-regex-safe')).default({ strict: false })

export default async function (m, conn = { user: {} }) {
  let _name 
  try { _name = await conn.getName(conn.decodeJid(m.sender)) } catch {}
  let sender = m.sender ? PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '') : 'Not detect'
  let chat = await conn.getName(m.chat)
  // let ansi = '\x1b['
  let img
  try {
    if (Helper.opts['img'])
      img = /sticker|image/gi.test(m.mtype) ? await buffer(await m.download()) : false
  } catch (e) {
    console.error(e)
  }
  let filesize = (m.msg ?
