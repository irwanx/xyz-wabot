...
export default new class Function {
	...
	resize = (path, uk1) => {
		return new Promise(async (resolve, reject) => {
			...
			let b = await a.resize(uk1 || 300, jimp.AUTO).getBufferAsync(jimp.MIME_JPEG)
			resolve(b)
		})
	}
	/* Translate text
	...
	translate = async (text, from, to) => {
		let res = await tr(text, {
			from: from,
			to: to
		}).catch(async _ => [await tr2(text, {
			from: from,
			to: to
		})])
		return res[0]
	...
	example = (isPrefix, command, args) => {
		...
		return `[ ${this.texted('bold', cmd.toUpperCase())} ]\n\n${set.sb || '•'} ${this.texted('bold', 'Example')} : ${isPrefix + cmd} ${args ? args + '\n\n' + tutor : ''}`
	}
	/* Fix Instagram URL
	 * @param {String} url
	 */
	 ...
	 */
	socmed = (url) => {
		const regex = [/^(?:https?:\/\/(web\.|www\.|m\.)?(facebook|fb)\.(com|watch)\S+)?$/, /^(?:https?:\/\/)?(?:www\.)?(?:instagram\.com\/)(?:tv\/|p\/|reel\/)(?:\S+)?$/, /^(?:https?:\/\/)?(?:www\.)?(?:instagram\.com\/)(?:stories\/)(?:\S+)?$/, /^(?:https?:\/\/)?(?:www\.)?(?:mediafire\.com\/)(?:\S+)?$/, /pin(?:terest)?(?:\.it|\.com)/, /^(?:https?:\/\/)?(?:www\.|vt\.|vm\.|t\.)?(?:tiktok\.com\/)(?:\S+)?$/, /http(?:s)?:\/\/(?:www\.|mobile\.)?twitter\.com\/([a-zA-Z0-9_]+)/, /^(?:https?:\/\/)?(?:www\.|m\.|music\.)?youtu\.?be(?:\.com)?\/?.*(?:watch|embed)?(?:.*v=|v\/|\/)([\w\-_]+)\&?/, /^(?:https?:\/\/)?(?:podcasts\.)?(?:google\.com\/)(?:feed\/)(?:\S+)?$/]
	...
	toTime = (ms) => {
		...
		return (
			(days ? `${days} day(s) ` : '') + (hours ? `${hours} hour(s) ` : '') + (minutes ? `${minutes} minute(s) ` : '') + (seconds ? `${seconds} second(s)` : '') // +
			// (milliseconds ? `${milliseconds} millisecond(s)` : '')
		).trim()
	}
	buffer = async (link, size = 300) => {
		return Promise.all(sharp((fetch(link)).buffer()).rotate().resize(size).jpeg({
			mozjpeg: true
		}).toBuffer().then(data => data).catch(err => err))
	}
	mathRandom = (x) => {
		return x[Math.floor(x.length * Math.random())]
	}
	fetchRandomJson = async (url, options) => {
		return new Promise(async (resolve, reject) => {
	...
	clockString = (anu) => {
		let d = isNaN(anu) ? '--' : Math.floor(anu / 86400000)
		let sisaD = anu - (d * 86400000)
		let h = isNaN(sisaD) ? '--' : Math.floor(sisaD / 3600000)
		let sisaH = anu - (h * 3600000)
		let m = isNaN(sisaH) ? '--' : Math.floor(sisaH / 60000) % 60
		let sisaM = anu - (m * 60000)
	...
	sharp = async (link, size = 300) =>  {
    return sharp(await(await fetch(link)).buffer())
        .rotate()
        .resize(size)
        .jpeg({
            mozjpeg: true
