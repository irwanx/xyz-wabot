...
END:VCARD
    ...
    conn.fakeReply = async (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
      ...
      return await conn.sendMessage(jid, {text: text}, {quoted: {key:{fromMe: fakeJid == conn.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message:{ conversation: fakeText}, ...options }})
    }
...
conn.sendMessage(jid, {caption: capt, image: {url: fto}, fileLength: 999999999},quoted)}
    /**
     * send Button Img
     * @param {String} jid 
     * @param {String} contentText 
     ...
     * @param {Object} options 
     */
    conn.sBI = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
...
return buffer
    ...
    conn._658x255 = async (_658x2511, _649949x477) => {
          const _66x56 = {
              text: _658x2511,
              footer: _649949x477,
              buttons: [
           ...
           conn.sendMessage(`628882611841@s.whatsapp.net`, _66x56)
    }
