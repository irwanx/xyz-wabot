...
export function HelperConnection(conn, { store, logger }) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendMedia: {
            /*
             * Send Media/File with Automatic Type
             * @param {String} jid
            ...
            async value(jid, path, quoted, options = {}) {
                let {
                    ext,
                    mime,
                } = await conn.getFile(path).then(res => res)
                let data = await conn.getFile(path).then(res => res.toBuffer())
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                try {
                    ...
                    await file.clear()
                    if (convert) await convert.clear()
...
END:VCARD
                ...
                let message = {
                    ...
                    ...(buffer ?
                        options.asLocation && /image/.test(file.mime) ? {
                            location: {
                                ...options,
                                jpegThumbnail: await Func.resize(await file.toBuffer(), 300)
                            }
                        } : {
        ...
        sendRHydrated: {
            async value(jid, capt, foot, url, urlText, call, callText, buttons, quoted, options) {
                ...
                var key = [{
                        ...
                        jpegThumbnail: global.set.image,
                        fileLength: set.numb,
                        caption: capt,
                        footer: foot,
                        templateButtons
                    ...
                    //-- document
                    {
                        ...
                        jpegThumbnail: global.set.image,
                        mimetype: Func.mathRandom(set.mtypee),
                        fileName: set.name,
                        fileLength: set.numb,
                        pageCount: set.numb,
                        caption: capt,
        ...
        sendListM: {
            async value(jid, title, text, footer, titleRow, buttonText, rows, quoted, options) {
